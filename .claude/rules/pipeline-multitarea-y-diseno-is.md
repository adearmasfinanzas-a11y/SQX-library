...
**Actualización 2026-07-13 — resuelta la pregunta de autonomía real:** el usuario preguntó si el asistente podía armar tareas del pipeline de forma completamente autónoma (editando el `project.cfx` directamente, sin la interfaz) para que la automatización de plantillas futuras fuera real. Un primer intento corrompió el proyecto real (ver incidente completo y protocolo de solución en `@.claude/rules/mecanismo-edicion-directa-proyectos.md`) — se restauró sin pérdida de datos y se encontró un método de reconstrucción de ZIP que sí funciona, validado dos veces (un proyecto de prueba descartable, y después aplicado de verdad al proyecto real agregando la tarea `Clear Databanks`, confirmado por el usuario en la interfaz). **Con esto, el paso 4 de esta metodología (implementar tras aprobación) puede hacerse editando XML directamente para cualquier plantilla futura, siguiendo el protocolo de esa regla — ya no depende de que el usuario arme cada tarea a mano en la interfaz.**
...
### 1d. El análisis de régimen de mercado se consulta ANTES y DURANTE el diseño, no solo al interpretar resultados raros (establecido 2026-07-13)

**Incidente que expuso este hueco real en el proceso:** al correr el primer Retest de `EURUSD-REVRANGE-H1-001` sobre la ventana OOS reservada (`2025.01.01`-`2026.04.13`), **cero estrategias pasaron el filtro** — la mayoría con Profit Factor razonable pero Ret/DD débil. Se investigó **después** de ver el resultado raro, y se encontró que EURUSD tuvo una tendencia alcista sostenida y muy fuerte durante casi toda esa ventana (de ~1.04 a ~1.20, ~15%, impulsada por debilidad del dólar y divergencia Fed/BCE) — un régimen estructuralmente adverso para una hipótesis de reversión en rango (PF razonable por ganancias pequeñas frecuentes, pero Ret/DD débil por drawdowns puntuales cuando el precio no revierte y sigue tendiendo). El patrón encaja mecánicamente con la evidencia de régimen, no parece ruido aleatorio.

**El usuario señaló correctamente que este tipo de análisis debe ser proactivo, no reactivo — se fija como principio permanente:**

1. **Al diseñar el Build** (ya cubierto parcialmente en la sección 1): investigar regímenes conocidos del activo antes de fijar el rango IS.
2. **Al elegir/reservar la ventana OOS del Retest** (hueco real detectado hoy, no se hizo antes de correr): investigar qué régimen tuvo el activo durante esa ventana específica **antes** de correrla, no después de ver un resultado sospechoso — la ventana se reservó únicamente por disponibilidad de datos (lo que no se le dio al Build), sin verificar si coincidía con un período estructuralmente favorable o adverso para la hipótesis concreta.
3. **Al diseñar la hipótesis/ficha misma** (conectar con `formato-entrega.md`, secciones 9-10, "Evaluación crítica de fortalezas y debilidades" y "Riesgos de sobreajuste específicos"): documentar explícitamente a qué régimen es estructuralmente vulnerable la hipótesis (ej. reversión en rango → vulnerable a tendencias sostenidas) como parte del diseño, no como hallazgo posterior sorpresivo.
4. **Al interpretar cualquier resultado de Retest/robustez:** cruzar siempre contra el contexto real de régimen de la ventana evaluada antes de sacar conclusiones sobre si "la hipótesis no tiene ventaja" — un resultado débil en una ventana con régimen adverso conocido no tiene el mismo peso que uno débil en una ventana de condiciones normales (mismo principio ya establecido en `criterio-descarte-plantillas.md`: distinguir "esta instancia falló" de "la hipótesis no tiene ventaja real", ahora con el régimen de mercado como una de las variables a revisar siempre).
...
**CORRECCIÓN IMPORTANTE tras el primer resultado real (2026-07-13): no mezclar el estrés de costos con la validación OOS primaria.** Primera corrida real de `EURUSD-REVRANGE-H1-001` con costos estresados: **0 de 535 pasaron** — la mayoría con Profit Factor razonable pero Ret/DD débil, patrón que coincidió con una tendencia sostenida fuerte de EURUSD en la ventana reservada (ver sección 1d). Se sospechó que el estrés de costos estaba pesando además del régimen adverso, y se corrió una segunda vez con los costos **reales** del Build (Commission=6, Swap=-6.5/-3.5, Spread=0.4, Slippage=0, todo lo demás igual) — **resultado: 39 de 535 pasaron.** Confirma que el estrés de costos era un factor dominante, no solo el régimen. **Se fija como corrección de diseño: el estrés de costos (~30-35%) no va en la misma pasada que decide supervivencia — se aplica DESPUÉS, como prueba de margen de seguridad separada sobre lo que ya sobrevivió la validación OOS con costos reales**, mismo principio de capas separadas ya usado para Monte Carlo/Walk-Forward Matrix (sección "Cross checks" más abajo). El Retest primario de cualquier plantilla futura debe usar los costos **reales** del broker objetivo, no estresados.
...
4. **Corolario de eficiencia (propuesto por el usuario, 2026-07-13): si el cross-check de mercado adicional no filtra nada, no tiene sentido correrlo temprano sobre un conjunto que todavía se va a achicar más.** Un backtest en otro mercado sobre todo el histórico no es barato (comparable en costo a un Retest completo) — correrlo en cada pasada intermedia (Monte Carlo, Walk-Forward Matrix) desperdicia cómputo en candidatas que van a descartarse de todos modos en pasadas posteriores. **Se fija: el cross-check de mercado adicional se desactiva en las pasadas intermedias, y se corre como paso final separado, dedicado, únicamente sobre el subconjunto final que sobreviva todo lo demás.** (No aplica retroactivamente a este primer Retest, donde ya corrió sobre las 39 supervivientes de esa etapa — el gasto ya está hecho, no hay que deshacerlo; aplica de acá en adelante.)
...
### Corrección importante: no encadenar los Retest como un embudo secuencial ciego (2026-07-13)

Se había propuesto (y se corrige acá) separar primero las 39 supervivientes del Retest 1 y correr el Retest 2 (Monte Carlo) solo sobre ellas, asumiendo que las 496 restantes "ya fallaron" y no valía la pena seguir evaluándolas. **El usuario señaló un error real en ese razonamiento:** el Retest 1 filtró específicamente por desempeño en **una ventana OOS puntual de 15 meses, ya identificada como un régimen de tendencia adverso para esta hipótesis** (sección 1d) — fallar esa prueba concreta no prueba que la estrategia carezca de base estadística sólida en general. Monte Carlo y Walk-Forward Matrix evalúan **dimensiones distintas de robustez** (aleatorización de operaciones, consistencia across múltiples ventanas históricas) — no son una versión "más estricta" de la misma prueba, y no hay evidencia de que quien falló el Retest 1 vaya a fallar también estas.

**Se corrige el diseño: las pasadas de robustez (Monte Carlo, Walk-Forward Matrix) se corren sobre el conjunto completo de 535 (`OOS_Filtrado`), no sobre un subconjunto pre-filtrado por el resultado de una prueba anterior de ventana única.** El resultado de cada Retest queda documentado como un dato más a considerar — la decisión final se toma con el cuadro completo de evidencia (Retest 1 + Monte Carlo + Walk-Forward + generalización a otro mercado), no por eliminación secuencial ciega. El costo computacional adicional de evaluar el conjunto completo en cada pasada es preferible al riesgo de descartar candidatas genuinamente robustas por el resultado de una sola ventana adversa.
