...
## 2026-07-12 — Primera tarea del pipeline (`Filter Strategies`): 535/1000 pasan el filtro OOS
- Armada en conjunto con el usuario desde la interfaz (ver `.claude/rules/mecanismo-condiciones-filtrado.md` para el detalle técnico completo de cómo se configuró y qué reveló sobre el motor de SQ). Acción `Moved`, `Results` → `OOS_Filtrado` (databank nuevo), sin tope de cantidad, condiciones: ProfitFactor (Out of Sample, ambas direcciones, dinero) ≥1.2 **y** NumberOfTrades (Out of Sample) ≥30.
...
## 2026-07-12 — Spread del Build confirmado como referencia de costo realista (no es plantilla de fondeo)
- Al armar el Retest, el usuario recordó que el spread del Build (`0.4` pips en `EURUSD_dukas_M1_UTCPlus02`) se había consultado en su momento con referencia a FTMO. Se confirmó por internet (2026-07-12): FTMO cotiza EURUSD con spread de 0.4 pips + ~$3 de comisión — coincide exactamente con el valor ya configurado, buen valor realista de referencia.
- **Aclarado explícitamente con el usuario:** esta plantilla **no** apunta a pasar un challenge de fondeo — FTMO se usó únicamente como referencia de costo realista/ajustado (spreads de tipo ECN confiables), no porque el objetivo sea superar una evaluación de FTMO. `EURUSD-REVRANGE-H1-001` sigue siendo una plantilla de largo plazo estándar; **no aplican** las reglas de `.claude/rules/plantillas-cuentas-fondeo.md` (ventanas cortas, tasa de éxito por ventana móvil, límite de pérdida diaria) — esa regla queda para cuando se aborde una plantilla de fondeo real, todavía no existe ninguna en la biblioteca.
- Fuentes del dato de spread: [Understanding Spreads and Commissions at FTMO](https://allproptradingfirms.com/understanding-spreads-and-commissions-at-ftmo/), [FTMO Spreads](https://fxverify.com/tools/prop-firm-spreads/ftmo-2509).

## 2026-07-13 — Cuarta tarea del pipeline (`Clear Databanks`) agregada de forma autónoma, sin interfaz
- El usuario pidió como prueba explícita que el asistente armara la tarea `Clear Databanks` (limpia `Results`, las 465 estrategias que no pasaron el primer filtro OOS, ya respaldadas) editando el `project.cfx` directamente, sin usar la interfaz.
- **Primer intento corrompió el proyecto** (SQ mostró "Config file is corrupted" al abrir) por un método de escritura de ZIP incompatible (`.NET ZipFile 'Update'` mode). Restaurado desde backup sin pérdida de datos reales — confirmado por el log de carga del proyecto (`OOS_Filtrado`: 535 estrategias, `Results`: 465 estrategias, ambos intactos).
- **Segundo intento, con método corregido** (reconstrucción completa del `.cfx` desde cero, validado antes en un proyecto de prueba descartable): funcionó — la tarea `Clear Databanks` quedó agregada correctamente entre `Filter strategies` y `Retest strategies`, confirmado por el usuario en la interfaz (posición correcta, `Databank name="Results"` bien cargado).
- Ver `.claude/rules/mecanismo-edicion-directa-proyectos.md` para el protocolo completo y el detalle técnico del incidente — queda como método estándar para armar pipelines de forma autónoma en plantillas futuras.
- **Secuencia actual del pipeline:** `Build → Save to files → Filter strategies → Clear Databanks → Retest strategies`, las 5 tareas activas.
...
## 2026-07-13 — Primer resultado real del Retest: 0 → 39 al corregir el estrés de costos
- **Primera corrida (costos estresados ~30%):** 0 de 535 pasaron los 4 filtros. Diagnóstico: la mayoría tenía Profit Factor(OOS) razonable (muchas >2, algunas >4) pero Ret/DD(OOS) débil (mayoría entre 1.1-2.2, umbral era ≥3). Se investigó por internet: EURUSD tuvo una tendencia alcista fuerte y sostenida durante casi toda la ventana OOS reservada (2025.01-2026.04, de ~1.04 a ~1.20, ~15%) — régimen estructuralmente adverso para una hipótesis de reversión en rango (PF razonable por ganancias frecuentes pequeñas, Ret/DD débil por drawdowns cuando el precio no revierte). Se registró como principio permanente investigar régimen de mercado ANTES de correr una ventana OOS, no después (ver `.claude/rules/pipeline-multitarea-y-diseno-is.md` sección 1d).
- **Segunda corrida (mismos parámetros, costos reales del Build — Commission=6, Swap=-6.5/-3.5, Spread=0.4, Slippage=0):** **39 de 535 pasaron.** Confirma que el estrés de costos era un factor dominante en el resultado de la primera corrida, además del régimen adverso. Se corrigió el diseño del pipeline: el estrés de costos no va mezclado en la validación OOS primaria, se aplica después como prueba de margen de seguridad separada sobre los sobrevivientes reales.
- **Estado real de la plantilla:** 39 candidatas sobrevivieron Build + filtro OOS interno + Retest OOS en ventana genuinamente nueva y adversa (con costos reales) — primer resultado sólido y honesto del pipeline completo.
- **Pendiente:** separar las 39 con `Filtering` (modo "Éxito"), y decidir el orden de las próximas pasadas (Monte Carlo, Walk-Forward Matrix, y ahora también una prueba de estrés de costos aparte sobre estas 39).
